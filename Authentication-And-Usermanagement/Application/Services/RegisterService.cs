...
namespace Application.Services
{
    public class RegisterService : IRegisterService
    {
        ...
        public RegisterService(IUserAuthenticationRepository userAuthentication, IMessagePublisher publisher, IAppLogger<RegisterService> logger)
        {
        ...
        public async Task<IdentityResult> Register(RegisterDto registerDto)
        {
            ...
            if (result.Succeeded)
            {
                ...
                await _publisher.PublishUserRegisterAsync(user.Email, user.UserName, user.FirstName, user.LastName);

                _logger.LogInformation("(register) Event sent to Notificatino service");
